...
await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/ARJUNSER.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted : message.data, ptt: true})
        }
...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '916238440754@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./stickers/arjunser.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: false})
    }
const array = ['Pinky','asena','ayin','back','Back','Bot','fuck','Fuck','Hehe','Hello','Hlo','Kill','kill','kiss','line','love','mwolu','Mwolu','single','tha','thund','z','Z','bie','Bie','uff','umma','Vaa','vada','yo','umbi','Shibhil','no girls','Love you','Girls','Devan','arjunser']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
